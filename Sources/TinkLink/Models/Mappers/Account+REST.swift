...
extension Account {
    init(restAccount account: RESTAccount) {
        ...
        self.identifiers = Self.makeTransferEntityURI(account.identifiers)
        ...
        self.flags = Account.makeAccountFlags(account.flags)
        self.accountExclusion = Self.AccountExclusion(restAccountExclusion: account.accountExclusion)
    ...
    static func makeAccountFlags(_ string: String?) -> [Account.Flag]? {
        guard let data = string?.data(using: .utf8), let restAccountFlags = try? JSONDecoder().decode([RESTAccount.Flag].self, from: data) else { return nil }
        return restAccountFlags.compactMap { Account.Flag(restAccountFlags: $0) }
    }

    static func makeTransferEntityURI(_ string: String?) -> [Transfer.TransferEntityURI]? {
        guard let data = string?.data(using: .utf8), let restIdentifiers = try? JSONDecoder().decode([String].self, from: data) else { return nil }
