...
final class RESTUserService: UserService {
    ...
    private let userRequestClientBehaviors: ComposableClientBehavior = ComposableClientBehavior(behaviors: [AuthorizationHeaderClientBehavior(sessionCredential: nil)])
    ...
    func createAnonymous(market: Market?, locale: Locale, origin: String?, completion: @escaping (Result<AccessToken, Error>) -> Void) -> RetryCancellable? {
        ...
        return client.performRequest(request, requestSpecificBehavior: userRequestClientBehaviors)
    }
