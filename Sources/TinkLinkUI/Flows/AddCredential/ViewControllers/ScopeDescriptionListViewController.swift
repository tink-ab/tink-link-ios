...
final class ScopeDescriptionListViewController: UITableViewController {
    ...
    init(authorizationController: AuthorizationController, scopes: [Scope]) {
        ...
        self.sections = [
            .intro(
                title: NSLocalizedString("AddCredentials.ScopeDescriptions.Title", tableName: "TinkLinkUI", value: "We’ll collect the following data from you", comment: "Title for text introducing the descriptions for which data points will be collected when using the service."),
                description: NSLocalizedString("AddCredentials.ScopeDescriptions.Body", tableName: "TinkLinkUI", value: "By following through this service, we’ll collect financial data from you. These are the data points we will collect from you:", comment: "Text introducing the descriptions for which data points will be collected when using the service.")
            )
        ]
