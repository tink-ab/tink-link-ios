...
final class AddCredentialSession {
    ...
    func addCredential(provider: Provider, form: Form, allowAnotherDevice: Bool, onCompletion: @escaping ((Result<AuthorizationCode, Error>) -> Void)) {
        ...
        self.showUpdating(status: NSLocalizedString("AddCredentials.Progress.Authorizing", tableName: "TinkLink", value: "Authorizing…", comment: "Text shown when adding credentials and waiting for authorization."))
    }
        ...
        case .awaitingThirdPartyAppAuthentication(let thirdPartyAppAuthenticationTask):
            if shouldAuthenticateInAnotherDevice {
                ...
                thirdPartyAppAuthenticationTask.openThirdPartyApp { [weak self] in
                    DispatchQueue.main.async {
                        self?.showUpdating(status: NSLocalizedString("AddCredentials.Progress.WaitingForAuthenticationOnAnotherDevice", tableName: "TinkLink", value: "Waiting for authentication on another device", comment: "Text shown when adding credentials and waiting for authenticvation on another device."))
                    }
                }
            }
...
extension AddCredentialSession: SupplementalInformationViewControllerDelegate {
    func supplementalInformationViewControllerDidCancel(_ viewController: SupplementalInformationViewController) {
        parentViewController?.dismiss(animated: true) {
            ...
            self.showUpdating(status: NSLocalizedString("AddCredentials.Progress.Canceling", tableName: "TinkLink", value: "Canceling…", comment: "Text shown when canceling supplementing information."))
        }
    }
